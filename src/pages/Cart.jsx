...
const Cart = () => {
  ...
  const { cart_products, successMessage, price, shipping_fee, outOfStockProduct, buy_product_item, loader } = useSelector(state => state.cart)
        ...
        <div className='w-[90%] xl:w-[70%] mx-auto py-10'>
          {
            loader ? (
              <div className='flex flex-wrap'>
                <div className='w-full lg:w-[70%] xl:w-[65%] flex flex-col gap-3'>
                  {[...Array(3)].map((_, i) => (
                    <div key={i} className='bg-white p-4 rounded-lg shadow-md flex gap-4'>
                      <Skeleton className="w-20 h-20 rounded-md" />
                      <div className='flex-1 flex flex-col gap-2'><Skeleton className="h-4 w-3/4" /><Skeleton className="h-4 w-1/4" /></div>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  <div className='w-full lg:w-[70%] xl:w-[65%]'>
                    <div className='pr-0 lg:pr-3'>
                      <div className='flex flex-col gap-3'>
                        ...
                        {
                          cart_products.map((p) => <div key={p._id || p.shopName} className='flex bg-[#eeeeee] md:bg-white p-4 flex-col gap-2 rounded-lg shadow-md'>
                            <div className='flex justify-start items-center'>
                              <h2 className='text-sm text-slate-600 font-bold'>{p.shopName}</h2>
                            </div>
                            {
                              p.products.map((item) => <div key={item._id} className='w-full flex flex-wrap '>
                        ...
                        }
                        {
                          outOfStockProduct.length > 0 && <div className='flex flex-col gap-3'>
                            ...
                            <div className='flex bg-[#eeeeee] md:bg-white p-4 flex-col gap-2 rounded-lg shadow-md'>
                              {
                                outOfStockProduct.map((item) => <div key={item._id} className='w-full flex flex-wrap '>
                                  <div className='flex w-full sm:w-7/12 gap-2 '>
                                    <div className='flex gap-4 justify-start items-center'>
