...
const OrderDetails = () => {
  ...
  return (
    <div className='bg-white p-5 rounded-md'>
      <h2 className='text-slate-600 mb-1'>Mã đơn hàng: # {myOrder._id}</h2>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 gap-3'>
        ...
        <div className='flex flex-col gap-3'>
          <h2 className='text-sm'>Giá: {myOrder.price}</h2>
          ...
          <p className='text-sm'> Trạng thái đơn hàng: <span className={`py-1 text-xs px-2 ${myOrder.delivery_status === 'placed' ? 'bg-green-300 text-green-800' : 'bg-red-300 text-red-800'} rounded-md`}>{translateDeliveryStatus(myOrder.delivery_status)}</span></p>
        </div>
      </div>
